...
class CGT(nn.Module):
    def __init__(self, edge_dim=14, out_dim=201*4, seed=123):
        ...
        # 118 elements, embedding dim 256
    ...
    def forward(self, data):
        ...
        # elec_conf = data.elec_conf # Not used in model?
        orbital_counts = data.orbital_counts
        batch = data.batch  
        ...
        # Node embedding
        # x shape might be [num_nodes, 1] or [num_nodes]
        if x.dim() > 1 and x.size(1) == 1:
            x = x.squeeze(1)
            
        # Ensure x is within range [0, 117] (atomic numbers 1-118 -> indices 0-117?)
        # Original code uses atomic_number directly (1-based).
        # Embedding(118) supports 0..117.
        # So max atomic number supported is 117.
        ...
        # orbital_counts is per node (atom)
        # We need to sum them up per graph
