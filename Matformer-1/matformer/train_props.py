...
def train_prop_model(
    ...
    if link is not None:
        config["model"]["link"] = link
    if per_bin_standardize:
        config["per_bin_standardize"] = True
    if weighted_loss:
        config["weighted_loss"] = True
    if fermi_weight_sigma is not None:
        config["fermi_weight_sigma"] = fermi_weight_sigma
    if fermi_bin_idx is not None:
        config["fermi_bin_idx"] = fermi_bin_idx
    if write_checkpoint is not None:
        config["write_checkpoint"] = write_checkpoint
