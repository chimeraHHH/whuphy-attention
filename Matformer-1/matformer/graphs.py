...
class PygStructureDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        # Print basic stats
        try:
            print("label mean %f std %f" % (float(self.labels.mean()), float(self.labels.std())))
        except Exception:
            pass
        # Normalize labels
        if mean_train is None:
            ...
            print("normalize using training mean %s and std %s" % (str(mean), str(std)))
        else:
            mt = mean_train
            st = std_train
            if not torch.is_tensor(mt):
                mt = torch.as_tensor(mt, dtype=self.labels.dtype)
            if not torch.is_tensor(st):
                st = torch.as_tensor(st, dtype=self.labels.dtype)
            self.labels = (self.labels - mt) / st
            try:
                print("normalize using training mean/std with shapes:", mt.shape, st.shape)
            except Exception:
                pass
...
def build_undirected_edgedata(
    ...
    u = torch.tensor(u)
    v = torch.tensor(v)
    r = torch.from_numpy(np.asarray(r)).type(torch.get_default_dtype())
