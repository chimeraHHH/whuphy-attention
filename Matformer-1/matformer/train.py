...
def train_dgl(
    ...
    if std_train is None:
        std_train = 1.0
    # derive scalar std for metrics if per-bin provided
    std_scalar = std_train
    try:
        import numpy as _np
    except Exception:
        _np = None
    if isinstance(std_train, (list, tuple)) and _np is not None:
        std_scalar = float(_np.mean(_np.array(std_train)))
    elif torch.is_tensor(std_train):
        if std_train.ndim > 0:
            std_scalar = float(std_train.mean().item())
        else:
            std_scalar = float(std_train.item())
    if model is None:
    ...
    # Optional Fermi-weighted loss for multi-output
    def make_weighted_mse(n_out, sigma=None, center=None):
        L = n_out
        if center is None:
            center = L // 2
        if sigma is None:
            sigma = max(1.0, L * 0.05)
        ...
        def _loss(y_pred, y_true):
            ...
            if y_pred.ndim == 1:
                ...
                yp = y_pred
                yt = y_true
            ww = w.unsqueeze(0).expand_as(yp)
            return torch.sum(ww * (yp - yt) ** 2) / torch.sum(ww)
        return _loss
    # decide criterion
    if (not classification) and (config.weighted_loss) and (config.model.output_features > 1):
        # parse sigma and center
        sigma = config.fermi_weight_sigma
        center = config.fermi_bin_idx
        # interpret sigma as fraction if <=1
        if sigma is not None:
            if sigma <= 1.0:
                sigma = max(1.0, config.model.output_features * sigma)
        criterion = make_weighted_mse(config.model.output_features, sigma=sigma, center=center)
    else:
        criterion = criteria[config.criterion]
    # set up training engine and evaluators
    metrics = {"loss": Loss(criterion), "mae": MeanAbsoluteError() * std_scalar, "neg_mae": -1.0 * MeanAbsoluteError() * std_scalar}
    ...
    if test_only:
        ...
        with torch.no_grad():
            for dat in test_loader:
                g, lg, lattice, target = dat
                try:
                    out_data = net([g.to(device), lg.to(device), lattice.to(device)])
                    success_flag=1
                except: # just in case
    ...
    # define default score function for checkpointing/early stopping
    if classification:
        my_metrics = "accuracy"
    else:
        my_metrics = "neg_mae"
    ...
    def log_results(engine):
        ...
        for metric in metrics.keys():
            ...
            if isinstance(vm, torch.Tensor):
                ...
                try:
                    import numpy as _np
                    if isinstance(vm, (_np.generic, _np.ndarray)):
                        vm = _np.array(vm).tolist()
                except Exception:
                    pass
            history["validation"][metric].append(vm)
                    ...
                    for item in seq:
                        try:
                            ...
                            if torch.is_tensor(item):
                                out[k].append(item.cpu().numpy().tolist())
                            else:
                                out[k].append(_np.array(item).tolist())
                        except Exception:
                            try:
                                out[k].append(float(item))
                            except Exception:
                                out[k].append(item)
                return out
            dumpjson(
                filename=os.path.join(config.output_dir, "history_val.json"),
                data=_jsonable_dict(history["validation"]),
            )
            dumpjson(
                filename=os.path.join(config.output_dir, "history_train.json"),
                data=_jsonable_dict(history["train"]),
            )
        if config.progress:
            pbar = ProgressBar()
            if not classification:
                try:
                    ...
                    elif isinstance(_val, (_np.generic, _np.ndarray)):
                        _val = float(_np.mean(_np.array(_val)))
                    if isinstance(_trn, torch.Tensor):
                        _trn = float(_trn.item())
                    elif isinstance(_trn, (_np.generic, _np.ndarray)):
                        _trn = float(_np.mean(_np.array(_trn)))
                except Exception:
                    _val = vmetrics['mae']
                    _trn = tmetrics['mae']
                pbar.log_message(f"Val_MAE: {_val:.4f}")
                pbar.log_message(f"Train_MAE: {_trn:.4f}")
            else:
    ...
    if (
        ...
        with torch.no_grad():
            ...
            for dat, id in zip(test_loader, ids):
                ...
                out_data = net([g.to(device), lg.to(device), lattice.to(device)])
                out_data = out_data.cpu().numpy().tolist()
                # ensure target is list
                target = target.cpu().numpy().flatten().tolist()
                # invert per-bin normalization if available
                try:
                    import numpy as _np
                    if isinstance(std_train, (list, tuple)):
                        out_data = list((_np.array(out_data) * _np.array(std_train) + _np.array(mean_train)).tolist())
                        target = list((_np.array(target) * _np.array(std_train) + _np.array(mean_train)).tolist())
                except Exception:
                    pass
                info = {}
                info["id"] = id
