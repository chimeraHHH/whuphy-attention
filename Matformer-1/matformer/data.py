...
def get_pyg_dataset(
    ...
    if mean_train is None:
        try:
            sample0 = vals[0]
            if per_bin_standardize and isinstance(sample0, (list, np.ndarray)):
                # Compute per-bin mean/std across training set
                stacked = np.stack([np.array(v, dtype=float) for v in vals])
                mean_train = stacked.mean(axis=0)
                std_train = stacked.std(axis=0)
                # Avoid zero std entries
                std_train[std_train == 0] = 1.0
            else:
                # Handle multi-output targets (list-of-lists) by flattening to scalars
                if isinstance(sample0, (list, np.ndarray)):
                    flat = []
                    for v in vals:
                    ...
                    mean_train = float(np.mean(flat))
                    std_train = float(np.std(flat))
                else:
                    mean_train = float(np.mean(vals))
                    std_train = float(np.std(vals))
        except Exception as exp:
            print("mean/std compute failed, defaulting to 0/1:", exp)
            mean_train = 0.0
            std_train = 1.0
        if isinstance(std_train, float) and std_train == 0:
            std_train = 1.0
...
def get_train_val_loaders(
    ...
    train_data, mean_train, std_train = get_pyg_dataset(
        ...
        use_angle=use_angle,
        use_save=False,
        per_bin_standardize=standard_scalar_and_pca is False and output_features > 1,
    )
